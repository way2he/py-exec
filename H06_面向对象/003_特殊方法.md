...
```
### 4.3 @staticmethod 与 @classmethod 的核心区别 
1. **参数传递规则**
- @staticmethod（静态方法） ：不接收类或实例的隐式参数，方法定义时不能包含 self 或 cls 参数（强行传参会报错）。
- @classmethod（类方法） ：必须接收第一个参数 cls （代表类本身），由解释器自动传递（类似实例方法的 self ）。

2. **调用方式**
两者均可通过 类名.方法() 或 实例.方法() 调用，但参数传递逻辑不同。

3. **典型使用场景**
- @staticmethod ：与类相关但无需访问类/实例状态的工具函数（如数学计算、格式校验）。
- @classmethod ：类级别的工厂方法（创建特定类型实例）、修改类共享属性（如类变量）。

### 4.4 `@property`：属性的访问控制
...
**通过 @<name>.deleter 可以自定义属性被删除时的逻辑（如资源释放、日志记录等），通常与 @property （获取属性）、 @<name>.setter （设置属性）配合使用，实现属性的完整封装。**
